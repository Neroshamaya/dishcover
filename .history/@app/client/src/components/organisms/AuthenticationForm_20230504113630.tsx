...
export default function AuthenticationForm({ onSubmit: postSubmit, login }: AuthenticationFormProps) {
  ...
  const onSubmit: SubmitHandler<SignUpFormSchemaType> = (userInfos) => {
      const response = login ? await authenticationService.login(userInfos) : await authenticationService.register(userInfos)
      if(!response?.error){
        context.setConnectedUser({...response?.data?.user, response?.data?token})
      }
